using SEDC.Homework.CSharp.Class08.Class;
using System;
using System.Collections.Generic;
using System.Linq;


namespace SEDC.Homework.CSharp.Class08
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            List<Song> allSongsStartingWithLetterB = Songs.Where(song => song.Title.StartsWith('B')).ToList();
            jerry.FavoriteSongs = allSongsStartingWithLetterB;
            jerry.GetFavSongs();
            Console.WriteLine();


            Person maria = FansArray.FirstOrDefault(name => name.FirstName == "Maria");
            List<Song> allSongsWithlengthLongerThanSixMinutes = Songs.Where(song => song.Length >= 360).ToList();
            maria.FavoriteSongs = allSongsWithlengthLongerThanSixMinutes;
            maria.GetFavSongs();
            Console.WriteLine();


            Person jane = FansArray.FirstOrDefault(name => name.FirstName == "Jane");
            List<Song> allSongsWithGenreRock = Songs.Where(song => song.Genre == Genre.Rock).ToList();
            jane.FavoriteSongs = allSongsWithGenreRock;
            jane.GetFavSongs();
            Console.WriteLine();


            Person stefan = FansArray.Find(person => person.FirstName == "Stefan");
            List<Song> allHipHopSongsUnderThreeMinutes = Songs.Where(song => song.Genre == Genre.Hip_Hop).Where(song => song.Length < 180).ToList();
            stefan.FavoriteSongs = allHipHopSongsUnderThreeMinutes;
            stefan.GetFavSongs();
            Console.WriteLine();


            List<Person> peopleWithMoreThanFourSongs = FansArray.Where(person => person.FavoriteSongs.Count >= 4).ToList();
            Console.WriteLine($"{peopleWithMoreThanFourSongs.Count} People who has have more than 4 songs:");

            foreach (var person in peopleWithMoreThanFourSongs)
            {
